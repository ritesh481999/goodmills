...
@section('title', __('common.farmer_group.farmer_group_update_page_title'))
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="kt-portlet">
                    <div class="kt-portlet__head">
                        <div class="kt-portlet__head-label">
                            <h3 class="kt-portlet__head-title">
                                {{__('common.farmer_group.update_farmer_group')}}
                            </h3>
                        </div>
                    ...
                    <!--begin::Form-->
                    {!! Form::model($group, ['route' => ['farmer_group.update', $group->id], 'class' => 'form-horizontal', 'method' => 'PATCH', 'id' => 'editFarmerGroup']) !!}
                    ...
                    <div class="kt-portlet__body">
                        <div class="kt-section kt-section--first">
                            <div class="form-group">
                                ...
                                {!! Html::decode(Form::label('name',  __('common.farmer_group.group_name').'<span class="text-danger">*</span>', ['class' => 'col-md-4 control-label'])) !!}
                                <div class="col-md-6">
                                    <input type="text" name="name" value="{{ $group ? $group->name : '' }}" id="name"
                                        class="form-control" placeholder="{{__('common.farmer_group.group_name_placeholder')}}" data-parsley-required="true"
                                        data-parsley-required-message="{{__('common.farmer_group.group_name_parsley_validation')}}"
                                        data-parsley-trigger="input blur" data-parsley-maxlength='100'
                                        data-parsley-pattern='{{ config('common.safe_string_pattern') }}'
                                        data-parsley-pattern-message="{{__('common.farmer_group.group_name_parsley_pattern_message')}}"
                                        data-parsley-maxlength-message='{{__('common.farmer_group.group_name_parsley_max_validation')}}'>
                                        ...
                                        <div class="col-md-3">
                                            <div class="custom-control custom-radio custom-control">
                                                <input type="radio" id="active-status" name="status"
                                                    class="custom-control-input" value="1" {{$group->status ? 'checked' : ''   }}>
                                        ...
                                        <div class="col-md-3">
                                            <div class="custom-control custom-radio custom-control">
                                                <input type="radio" id="inactive-status" name="status"
                                                    class="custom-control-input" value="0" {{$group->status ? '' : 'checked'   }}>
                            ...
                            <div class="">
                                ...
                                <input type="text" style="display: none;" name="farmer_ids" id="farmer_ids" value=""
                                    ...
                                    data-parsley-required-message='{{__('common.farmer_group.farmer_ids_parsley_validation')}}'
...
@section('script')
    ...
    <script type="text/javascript">
        var farmer_id = "{{ $group->id }}"
        var editRoute = '{{ route('farmer_group.edit', ':id') }}';
        var editUrl = editRoute.replace(':id', farmer_id);
        (function() {
            ...
            let ids = {!! json_encode($farmers) !!}
                .map(row => parseInt(row.id));
			$('#farmer_ids').val(ids);	
            $('#editFarmerGroup').parsley();

            const farmer_added_table = $('#farmer_added').DataTable({
                order: [
                    [1, "desc"]
                ],
                data: {!! json_encode($farmers) !!},
                columns: [{
                        data: 'id',
                        name: 'id'
                    },
